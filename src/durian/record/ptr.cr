class Durian::Record
  ...
  def self.{{name.id}}_from_io?(protocol : Protocol, resource_record : PTR, io : IO, buffer : IO, maximum_length : Int32 = 512_i32)
    data_length = io.read_bytes UInt16, IO::ByteFormat::BigEndian
    buffer.write_bytes data_length, IO::ByteFormat::BigEndian

    resource_record.domainName = PTR.address_from_io? protocol, io, data_length, buffer, maximum_length
  end
  {% end %}
