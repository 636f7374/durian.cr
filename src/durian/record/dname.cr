struct Durian::Record
  struct DNAME < Durian::Record
    ...
    def self.{{name.id}}_from_io?(protocol, io : IO, buffer : IO, maximum_depth : Int32 = 65_i32)
      resource_record = new
      data_length = io.read_bytes UInt16, IO::ByteFormat::BigEndian
      buffer.write_bytes data_length, IO::ByteFormat::BigEndian

      resource_record.delegationName = DNAME.address_from_io? protocol, io, data_length, buffer, maximum_depth
      resource_record
    end
    {% end %}
